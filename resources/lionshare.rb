...
env = environment('lionshare-slack') {
  region      'us-east-1'
...
project = project('coinbase', 'lionshare-slack') {
  environments 'lionshare-slack'
  tags {
    ProjectName "coinbase/lionshare-slack"
    self[:org] = "coinbase"
    self[:project] = "lionshare-slack"
  }
}
...
project.dynamo_table = project.resource("aws_dynamodb_table", "lionshare-slack") {
  name            "lionshare-slack"
  read_capacity   1
  write_capacity  1
  hash_key        "team_user_sha"
...
json_rest_api = project.from_template("json_rest_api", lambda_name, {
  lambda: {
    lionshare_slack: lambda_function
  },
  methods: {
    createp: {
      path: 'ls',
      ...
      api_key: false,
      auth: "NONE",
      handler: :lionshare_slack # reference to lambda above
    },
    oauth: {
      path: 'oauth',
      method: "GET",
